def read_graph():
    ...
    list1 = [[] for _ in range(verticies)]
    for _ in range(edges):
        v1, v2 = map(int, input().split())
        list1[v1 - 1].append(v2 - 1)
    return list1


def basic_search(adjacent_lists, visited, leave_sequences, v):
    visited[v] = True
    for adj in adjacent_lists[v]:
        if not visited[adj]:
            basic_search(adjacent_lists, visited, leave_sequences, adj)
    leave_sequences.append(v)


def reverse_adjacency_list(adjacent_lists):
...
def dfs(adjacent_lists, sources=None):
    ...
    for source in sources:
        if not visited[source]:
            tree_num += 1
            basic_search(adjacent_lists, visited, leave_sequences, source)
    return leave_sequences, tree_num


def find_strongly_connected_components(adjacent_lists):
    leave_sequences = dfs(adjacent_lists)[0]
    return dfs(reverse_adjacency_list(adjacent_lists), leave_sequences[::-1])[1]
